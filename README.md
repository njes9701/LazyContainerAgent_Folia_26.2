...
> ⚠️ **重要警示**
>
> - 此版本為 **Folia 26.2 / Java 25 專用版本**。
> - 本版本經由 **Codex 協助修改與移植**，請務必先在測試服驗證後再上正式服。
> - 已經過實際測試，確認 agent 能正常注入 Folia 26.2，並可有效減少大量容器載入/回存造成的伺服器效能消耗。
> - 第一次使用請先開啟 `shadow` 模式，確認 `shadowMismatch=0` 後，再切換到正式效能模式。

> **箱子物品「延遲反序列化 + 沒碰過就原樣寫回」的 Java agent。**
> 針對 Folia 26.2,把 chunk 載入時「立刻把每個箱子的物品從 NBT 解包」與卸載時「重新打包」這兩筆白工砍掉。

⚠️ **這不是外掛(plugin),是 Java agent** —— 用 `-javaagent:` 掛在 JVM 上,**不要丟 `plugins/`**(丟了沒用)。

> 🔒 **版本敏感(務必先讀)**
> 本 agent 以 bytecode **直接織入 Folia 26.2 / Java 25** 的內部類別(template classfile major 69),屬**版本綁死**的工具。
> - **僅適用於 Folia 26.2 + Java 25。** 任何其他 Minecraft 版本或 Java 版本,**一律不要直接套用**。
...
> 前提:**Folia 26.2 + Java 25**(其他版本請先看上面的「版本敏感」)。
...
1. **`LazyContainerAgentMain`**(premain):把整個 jar `appendToBootstrapClassLoaderSearch` 掛上 bootstrap classloader(繞過 Folia/Paper 隔離 classloader),再註冊 transformer。
...
- **Folia 26.2 目標**:本版以 build-only 相容為驗證範圍,要求 template 能對 Folia 26.2 NMS 編譯並產出 jar。舊 Paper 26.2 端對端/shadow 資料只作背景參考;詳見 [`docs/test-reports/26.2.md`](docs/test-reports/26.2.md)。
...
```bash
bash build.sh        # JDK 25 + Folia 26.2 NMS;產出 target/LazyContainerAgent.jar
```
需要 `nms-lib/`(你的 Folia 26.2 伺服器核心 NMS 編譯相依 libraries,供 `template/` 對真實 NMS 編譯)。
...
舊 Paper 正式環境實掛 shadow 模式時,觀察到的背景資料:
...
- **版本綁定**:Folia **26.2 / Java 25**(template major 69)。換版需用對應 NMS 重編 `template/`,並把 ASM 升到能讀目標 classfile 版本。版本不符會在開機/首次載箱子時**大聲報錯**(VerifyError/NoSuchMethod),不會靜默毀資料。詳見下方「版本敏感(務必先讀)」。
